...
#ifndef SRC_BACNET_APDU_FRAME_DETAIL_abort_GRAMMAR_HPP_
#define SRC_BACNET_APDU_FRAME_DETAIL_abort_GRAMMAR_HPP_
...
struct abort_grammar : grammar<Iterator, abort() >{

  rule<Iterator, abort()>                            abort_rule;
  rule<Iterator, pdu_type_and_control_information_t()>    pdu_header_rule;
  rule<Iterator, uint8_t()>                               original_invoke_id_rule;
  rule<Iterator, uint8_t()>                               abort_reason_rule;
  ...
  abort_grammar() : abort_grammar::base_type(abort_rule){

    abort_rule  = pdu_header_rule
    ...
    abort_reason_rule         = byte_;


    pdu_type_check_rule       = ( eps(boost::phoenix::bind(&pdu_type_and_control_information_t::pdu_type, ref(pdu_header_)) == pdu_type::abort)
...
#endif /* SRC_BACNET_APDU_FRAME_DETAIL_abort_GRAMMAR_HPP_ */
