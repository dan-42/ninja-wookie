...
BOOST_AUTO_TEST_CASE( test_send_broadcast ) {
  ...
  transport_.register_receive_callback([](bacnet::error ec, bacnet::common::protocol::mac::address adr, bacnet::binary_data data){
  ...
  npdu_controller_.async_send_broadcast(data_to_send, [](const bacnet::error &ec) {
  });
...
BOOST_AUTO_TEST_CASE( test_send_unicast ) {
  ...
  data_to_send.push_back(0x10); //unconfirmed service
  data_to_send.push_back(0x08); //who is
  npdu_controller_.async_send_unicast(endpoint, data_to_send, [](const bacnet::error &ec) {
  });
...
BOOST_AUTO_TEST_CASE( test_receive_broadcast ) {
  ...
  auto async_receive_callback_ = [](bacnet::error ec, bacnet::common::protocol::mac::address adr, bacnet::binary_data data){
  };
